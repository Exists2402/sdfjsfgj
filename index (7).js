...
( () => {
    ...
    var t, e, i, s, a = {
                    ...
                    case s.fh.CHAT_MESSAGE:
                        {
                            ...
                            if (e.startsWith("/")) {
                                ...
                                if (e.startsWith("/spawnmob")) {
                                    const args = e.split(" ")
                                    if (this.auth.username !== "Igor" && this.auth.username !== "Igormain") this.systemMessage("You are not the dev")
        ...
        446: (t, e, i) => {
            ...
            const r = [new s.XE("Ladybug",25,10,25,2.5).setNeutral(1).addDrop(h("Light")).addDrop(h("Rose"), .6), new s.XE("Rock",75,5,27.5,0).addDrop(h("Rock")).addDrop(h("Heavy"), .5, 2).addDrop(h("Moon"), .005, 6), new s.XE("Bee",15,25,25,4).setMoveInSines(1).setNeutral(1).addDrop(h("Stinger"), .7).addDrop(h("Pollen")).addDrop(h("Honey"), .4), new s.XE("Spider",20,10,20,4).setAggressive(1).setPoison(5, 3).setProjectile({
                petalIndex: h("Web") + 1,
                cooldown: 22.5,
                health: 1 / 0,
                damage: 0,
        ...
        904: (t, e, i) => {
            ...
            class l {
                ...
                update(t, e, i) {
                    ...
                    let baseOrbitSize = moon?.config?.size ?? this.player.size;
                    if (moon) baseOrbitSize *= 6;
                    ...
                    for (let i = 0; i < this.amount; i++) {
                        ...
                        if (o) {
                            ...
                            o.range <= 0)) {
                                ...
                                if (t.config.name === "Beetle") t.health.maxHealth = 1800 * Math.pow(3, this.rarity), t.health.health = t.health.maxHealth, t.damage = 15 * Math.pow(3, this.rarity);
                                if (t.config.name === "Soldier Ant") t.health.maxHealth = 500 * Math.pow(3, this.rarity), t.health.health = t.health.maxHealth, t.damage = 18 * Math.pow(3, this.rarity);
                                if (t.config.name === "Sandstorm") t.health.maxHealth = 60 * Math.pow(3, this.rarity), t.health.health = t.health.maxHealth, t.damage = 60 * Math.pow(3, this.rarity);
            ...
            class g extends c {
                ...
                update() {
                    ...
                    this.parent.type === s.wv.PLAYER && this.parent.defend && this.burst.ticks <= 0)) {
                        ...
                        if (moon) {return moon.velocity.x += Math.cos(t) * (e / 2),
                            moon.velocity.y += Math.sin(t) * (e / 2),
                        void this.destroy()}
                        else {
                ...
                collide() {
                    if (this.config?.name && this.config?.name === "Moon") {
