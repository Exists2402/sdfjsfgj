...
( () => {
    ...
    var t, e, i, s, a = {
                    ...
                    case s.fh.CHAT_MESSAGE:
                        {
                            ...
                            if (e.startsWith("/")) {
                                if (e.startsWith("/drops")) {
                                    ...
                                    this.systemMessage(`${h.ey[h.ey.findIndex(m => m.name.toLowerCase() === name.toLowerCase()) ?? 0].name}:`)
                                    for (const i of h.ey[h.ey.findIndex(m => m.name.toLowerCase() === name.toLowerCase()) ?? 0].drops) {
                                ...
                                if (e.startsWith("/craft")) {
                                    ...
                                    if (success === 1) {
                                        return this.systemMessage(`You have crafted a ${nameRaritys[indexRarity + 1]} ${namePetals[petalID]}`)
                                    }
                                    else if (success > 1) {
                                        return this.systemMessage(`You have crafted ${success} ${nameRaritys[indexRarity + 1]} ${namePetals[petalID]}'s`)
                                    }
                                    else {return this.systemMessage(`You have failed ${fail} attempts.`)}
        ...
        446: (t, e, i) => {
            ...
            const r = [new s.XE("Ladybug",25,10,25,2.5).setNeutral(1).addDrop(h("Light")).addDrop(h("Rose"), .6), new s.XE("Rock",75,5,27.5,0).addDrop(h("Rock")).addDrop(h("Heavy"), .5, 2), new s.XE("Bee",15,25,25,4).setMoveInSines(1).setNeutral(1).addDrop(h("Stinger"), .7).addDrop(h("Pollen")).addDrop(h("Honey"), .4), new s.XE("Spider",20,10,20,4).setAggressive(1).setPoison(5, 3).setProjectile({
                petalIndex: h("Web") + 1,
                cooldown: 22.5,
                health: 1 / 0,
                damage: 0,
                                    ...
                                    for (let step = 0; step < iterations; step++) {
                                        ...
                                        if (isPetalVsMob) {
                                            if (this.type === s.wv.PETAL && t.type === s.wv.MOB) {
                                                ...
                                                damageToT = petalRatio * this.damage;
                                                ...
                                                damageToThis = petalRatio * t.damage;
                                            }
                                        } else {
                                            damageToT = this.damage;
                                            damageToThis = t.damage;
