    ...
    var t, e, i, s, a = {
                            ...
                            if (s.A.gamemode === a.LX.MAZE) {
                                if (Math.random() >= 0.7) {
                                        ...
                                        if (void 0 !== e.tile?.spawn) {
                                            ...
                                            if (i && i.availableMobs.length) {
                                                ...
                                                for (let z = 0; z < i.availableMobs.length; z++) {
                                                    ...
                                                    if (weight < 0) {
                                                        mob = n.ey[i.availableMobs[z].mobId]
                                                        break
                                                    }
                                                }
                                    ...
                                    if (s.A.livingMobCount < s.A.maxMobs) {
                                        new h.Bw(e.position).define(mob, newrarity),
        
                                        newrarity >= s.A.announceRarity && s.A.announceRarity > -1 && (n.cK[newrarity] ? s.A.clients.forEach((i => i.systemMessage((0,
        
                                        l.Br)(n.cK[newrarity].name, !0) + " " + mob.name + " has spawned!", n.cK[newrarity].color))) : console.error(`Rarity returns undefined: ${e.rarity}`))}
                            }
                            }
                            }
                                ...
                                const mob = n.ey[c()]
                                  ...
                                  , e = s.A.lineMapMobSpawn(mob);

                                new h.Bw(e.position).define(mob, e.rarity)
        ...
        110: (t, e, i) => {
            ...
            class h {
                ...
                setExtraAttraction(t) {
                    for (let e = 0; e < this.tiers.length; e++)
                        this.tiers[e].extraAttraction = t * (e + 1);
                    return this
                }
                setWearable(t) {
                    ...
                    case s.fh.CHAT_MESSAGE:
                        {
                            ...
                            if (e.startsWith("/")) {
                                if (e.startsWith("/drops")) {
                                    ...
                                    const dropMulti = dropMultis[this.rarity];
                                    ...
                                    if (h.ey.findIndex(m => m.name.toLowerCase() === name.toLowerCase()) === -1) return this.systemMessage("Invalid Mob")
                                        ...
                                        for (let j = 0; j < 1 + (table.maxRarity - table.minRarity); j++) {
                                            ...
                                            chances.push({chance: Math.round((weight[j]/total)*100000)/1000, rarity: j + table.minRarity, amount: dropMulti[j]})
                                        ...
                                        chances.forEach(item => {
                                            this.systemMessage(`${item.amount}X ${h.cK[item.rarity].name}: ${item.chance}%`,h.cK[item.rarity].color);
                                        });
        ...
        446: (t, e, i) => {
            ...
            const r = [new s.XE("Ladybug",60,10,25,2.5).setNeutral(1).addDrop(h("Light"), .5).addDrop(h("Rose"), .5), new s.XE("Rock",75,10,27.5,0).addDrop(h("Rock"), .25).addDrop(h("Heavy"), .02, 2).addDrop(h("Moon"), .002, 6), new s.XE("Bee",35,50,25,4).setMoveInSines(1).setNeutral(1).addDrop(h("Stinger"), .5).addDrop(h("Pollen"), .25).addDrop(h("Honey"), .25), new s.XE("Spider",60,15,20,4).setAggressive(1).setPoison(5, 3).setProjectile({    petalIndex: h("Web") + 1,
                ...
                runs: !0,
                nullCollision: !0
            }).addDrop(h("Faster"), .5).addDrop(h("Web"), .5).addDrop(h("Third Eye"), .025, 5), new s.XE("Beetle",30,10,30,3).setAggressive(1).addDrop(h("Beetle Egg"), 1).addDrop(h("Lentil"), 0.25), new s.XE("Leafbug",35,3.5,30,2.5).setNeutral(1).setDamageReduction(.13).addDrop(h("Leaf")).addDrop(h("Bone"), .5).addDrop(h("Cactus"), .25), new s.XE("Roach",30,5,30,5.5).setNeutral(1).addDrop(h("Antennae"), 1, 2).addDrop(h("Magnolia"), .6).addDrop(h("Bone"), .6), new s.XE("Hornet",60,50,30,3).setAggressive(1).setProjectile({    petalIndex: h("Missile"),
                cooldown: 45,
                health: 5,
                damage: 10,
                speed: 5,
                range: 40
            }).addDrop(h("Missile"), .5).addDrop(h("Antennae"), .25, 2).addDrop(h("Orange"), .5), new s.XE("Mantis",35,10,32.5,2).setAggressive(1).setProjectile({    petalIndex: h("Pea.projectile"),
                cooldown: 140.625,
                health: 1.25,
                damage: 1.5,
                speed: 4.5,
            ...
            }).addDrop(h("Rock")).addDrop(h("Wing")).addDrop(h("Heavy"), .5, 2), new s.XE("Sandstorm",75,40,35,3).setSandstormMovement(1).setSize(35, s.rx.SIZE_SCALE, .9, .25).addDrop(h("Sand"), .25).addDrop(h("Glass"), .25).addDrop(h("Stick"), .1, 3), new s.XE("Scorpion",100,15,32.5,3).setAggressive(1).setStrafes(30, 15, 1.25).setProjectile({    petalIndex: h("Scorpion Missile.projectile"),
                cooldown: 45,
                health: 2,
                damage: 2,
                speed: 5,
                range: 65,
                size: .2
            }).addDrop(h("Pincer"), .5).addDrop(h("Iris"), .5), new s.XE("Demon",100,7.5,35,1).setAggressive(1).setPushability(.8).setProjectile({    petalIndex: h("Missile"),
                cooldown: 112.5,
                health: 1,
                damage: 1,
                speed: 5,
        ...
        904: (t, e, i) => {
            ...
            class l {
                ...
                define(t, e=0) {
                    ...
                    this.rarity = e,
                    this.config.tiers[this.rarity].extraAttraction && (this.player.extraAttraction += this.config.tiers[this.rarity].extraAttraction),
                ...
                update(t, e, i) {
                    ...
                    for (let i = 0; i < this.amount; i++) {
                        ...
                        if (o) {
                            const radius = 16 + this.player?.extraAttraction;
                            let target = undefined;
                            let distance = 64e8;
                            const targets = h.A.spatialHash.retrieve({_AABB: {
                                            x1: o.x - radius,
                                            y1: o.y - radius,
                                            x2: o.x + radius,
                                            y2: o.y + radius
                                        }}).forEach(t => {
                                            if (this.player.team !== o.team && t.type === s.wv.MOB && Math.pow(t.y - o.y, 2) + Math.pow(t.x - o.x, 2) < distance) {
                                                target = t;
                                                distance = Math.pow(t.x - o.x, 2) + Math.pow(t.y - o.y, 2)
                                            }
                                        })
                            ...
                            o.range <= 0 && (this.player.attack || this.player.defend))) {
                                ...
                                t.size = e.tiers[t.rarity].sizeRatio * Math.pow(1.3, this.rarity),
                            ...
                            if (!o.launched) {
                                ...
                                if (target && Math.pow(r + radius, 2) > Math.pow(this.player.x - o.x, 2) + Math.pow(this.player.y - o.y, 2)) {
                                    const dx = target.x - o.x;
                                    const dy = target.y - o.y;

                                    o.moveAngle = Math.atan2(dy, dx);
                                    o.moveStrength = Math.max(1, Math.cbrt(dx * dx + dy * dy) / o.speed)
                                }
                                else {
            ...
            class g extends c {
                ...
                define(t, e) {
                    ...
                    this.index = t.id,
                    this.spinSpeed = t.launchable ? 0 : (this.config.name === "Moon" ? .02 : .1),
                    this.armor = 0,
                    ...
                    if (t.forEach((t => {
                        if (t.clientID > 0) {
                            ...
                            looters?.forEach(player => {
                                if (player && !gotloot.includes(player.auth.username)) {
                                    gotloot.push(player.auth.username)
                                    ...
                                    for (const i of n.ey[this.index].drops) {
                                        ...
                                        t.push(new f(this,player,i.index,final + table.minRarity, dropMulti[final]))
